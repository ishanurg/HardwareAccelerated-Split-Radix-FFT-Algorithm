...
typedef struct {
    ...
    XSr_fft_uio_map maps[ MAX_UIO_MAPS ];
} XSr_fft_uio_info;

/***************** Variable Definitions **************************************/
static XSr_fft_uio_info uio_info;
...
static int uio_info_read_name(XSr_fft_uio_info* info) {
    char file[ MAX_UIO_PATH_SIZE ];
    sprintf(file, "/sys/class/uio/uio%d/name", info->uio_num);
    return line_from_file(file, info->name);
}

static int uio_info_read_version(XSr_fft_uio_info* info) {
    char file[ MAX_UIO_PATH_SIZE ];
    sprintf(file, "/sys/class/uio/uio%d/version", info->uio_num);
    return line_from_file(file, info->version);
}

static int uio_info_read_map_addr(XSr_fft_uio_info* info, int n) {
    int ret;
    char file[ MAX_UIO_PATH_SIZE ];
...
static int uio_info_read_map_size(XSr_fft_uio_info* info, int n) {
    int ret;
    char file[ MAX_UIO_PATH_SIZE ];
...
int XSr_fft_Initialize(XSr_fft *InstancePtr, const char* InstanceName) {
	XSr_fft_uio_info *InfoPtr = &uio_info;
